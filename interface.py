...
def get_CreditCardApproval():
    ...
    DAYS_BIRTH =eval(["DAYS_BIRTH"])
    DAYS_EMPLOYED =eval(["DAYS_EMPLOYED"])
    FLAG_MOBIL =int(data["FLAG_MOBIL"])
    FLAG_WORK_PHONE =int(["FLAG_WORK_PHONE"])
    FLAG_PHONE =int(["FLAG_PHONE"])
    FLAG_EMAIL =int(["FLAG_EMAIL"])
    OCCUPATION_TYPE =data(["OCCUPATION_TYPE"])
    CNT_FAM_MEMBERS =int(data["CNT_FAM_MEMBERS"])
    ...
    # else:
    #     return jsonify({"Result":f"Credit Card Approval Prediction is Decline"})
